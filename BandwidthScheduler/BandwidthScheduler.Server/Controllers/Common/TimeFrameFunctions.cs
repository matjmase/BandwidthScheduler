...
namespace BandwidthScheduler.Server.Controllers.Common
{
    public static class TimeFrameFunctions
    {
        ...
        public static bool StitchSidesCommitment(int userId, DateTime windowStart, DateTime windowEnd, Commitment encapsulate, Commitment left, Commitment right, DbSet<Commitment> db, out Commitment leftResult, out Commitment rightResult)
        {
            return StitchSides(windowStart, windowEnd, encapsulate, left, right, e => e.StartTime, e => e.EndTime, (s, e) => new Commitment() { UserId = userId, StartTime = s, EndTime = e }, e => db.Remove(e), out leftResult, out rightResult);
        }
